...
We wanted to do everything so that our project fulfills our main condition: so that the user can set the length of the dot and dash through a constant, for this we naturally needed a clock_divider in which there will be a constant depending on which the length of the dot and dash will be planned. And to display all the symbols to the user, we definitely had to make a driver_display that would process the signal and show which symbols were entered.
